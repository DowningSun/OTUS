...
R23 будет NTP server в Интернет со Stratum 1. IP адрес для обращения будет 150.150.150.1 - это PI адрес Триады. R23 взят просто в качестве примера.

       ntp master 1

На роутерах офиса Москвы задаем временную зону MSK +3 часа

        сlock timezone MSK 3

На роутерах R12 и R13 настроим обращение к NTP серверу в Интернет. В моем случае к R23 150.150.150.1

100.100.20.12/32 - это PI адрес AS1001 созданный на loopback1 R12 который доступен из Интернет

100.100.20.13/32 - это PI адрес AS1001 созданный на loopback1 R13 который доступен из Интернет

       ntp source loopback 1
       ntp server 150.150.150.1 prefer
       ntp master 2

На роутерах R14 и R15 выполняем следующие команды:

       ntp source Loopback0
       ntp server 12.12.12.12 prefer
       ntp server 13.13.13.13

![изображение](https://github.com/user-attachments/assets/38dcfce5-e31f-4df8-8f81-9eadd0c9aa28)
