...
**sticky MAC-адреса, преобразуются из динамически изученных адресов и добавляются в текущую конфигурацию сохранением и привязкой к порту.**

2. Что касается безопасности порта на S2, если вы загружаете скрипт текущей конфигурации на S2, почему порту 18 на PC-B никогда не получит IP-адрес через DHCP?
**Исходя из того, как я понял вопрос, имеется в виду конфигурация, в которой включен port-securoty, но нет запиесей о mac-адресах. Соответственно, поскольку mac-адрес PC-B будет неизучен, то порт будет уходить в режим блокировки, и устройство не сможет получить адрес.В остальных же случаях, поскольку у нас включен режим STICKY, то mac-адрес будет пррописан в коммутатор, и до момента переполнения допустимого количества Mac-адресов, проблем возникать не должно.**

3. Что касается безопасности порта, в чем разница между типом абсолютного устаревания и типом устаревание по неактивности?
**В абсолютном режиме мы сами задаем время устаревания адреса, после которого он будет удален. В режиме "по неактивности" защищенные адреса удаляются только еслли они неактивны**
