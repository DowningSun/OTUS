...
| SW2 | SVI VLAN 80 | 10.10.8.2 | 255.255.255.0 | 10.10.8.254 |
| SW3 | SVI VLAN 80 | 10.10.8.3 | 255.255.255.0 | 10.10.8.254 |
| SW4 | SVI VLAN 80 | 10.10.8.4 | 255.255.255.0 | 10.10.8.254 |
| SW5 | SVI VLAN 80 | 10.10.8.5 | 255.255.255.0 | 10.10.8.254 |
| SW6 | SVI VLAN 80 | 10.10.8.6 | 255.255.255.0 | 10.10.8.254 |
| SW7 | SVI VLAN 80 | 10.10.8.7 | 255.255.255.0 | 10.10.8.254 |
| SW8 | SVI VLAN 80 | 10.10.8.8 | 255.255.255.0 | 10.10.8.254 |
| SW9 | SVI VLAN 80 | 10.10.8.9 | 255.255.255.0 | 10.10.8.254 |
| SW10 | SVI VLAN 80 | 10.10.8.10 | 255.255.255.0 | 10.10.8.254 |
| SW11 | SVI VLAN 80 | 10.10.8.11 | 255.255.255.0 | 10.10.8.254 |
| SW12 | SVI VLAN 80 | 10.10.8.12 | 255.255.255.0 | 10.10.8.254 |
...
Для реализации требований заказчика по обесбечению изоляции сегметов сети используются расширенные списки доступа.

**Типовой список доступа для VLAN SALES, OPERATORS, OTHER, WH. Разница для этих VLAN будет только в адресе источника, соответствующем сети сегмента.**

permit udp any any eq bootps // разрешаем протокол DHCP

permit udp 10.10.1.0 0.0.0.255 host 192.168.10.253 eq domain

permit TCP 10.10.1.0 0.0.0.255 host 192.168.10.253 eq domain // разрешаем DNS

permit TCP 10.10.1.0 0.0.0.255 10.10.4.0 0.0.0.255 // разрешаем доступ в сеть принтеров

permit ICMP 10.10.1.0 0.0.0.255 10.10.4.0 0.0.0.255  // разрешаем ICMP трафик в сеть принтеров

deny ip 10.10.1.0 0.0.0.255 10.10.2.0 0.0.0.255  //запрещаем доступ в соседнюю сеть

deny ip 10.10.1.0 0.0.0.255 10.10.3.0 0.0.0.255  //запрещаем доступ в соседнюю сеть

deny ip 10.10.1.0 0.0.0.255 10.10.5.0 0.0.0.255  //запрещаем доступ в соседнюю сеть

permit ip 10.10.1.0 0.0.0.255 10.10.6.0 0.0.0.255 //разрешаем смотреть в сеть DMZ

deny ip 10.10.1.0 0.0.0.255 10.10.7.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.1.0 0.0.0.255 10.10.8.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.1.0 0.0.0.255 10.10.9.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.1.0 0.0.0.255 10.10.10.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

permit TCP 10.10.1.0 0.0.0.255 host 192.168.10.239 eq 3389 //разрешаем доступ к серверу по RDP 

permit TCP 10.10.1.0 0.0.0.255 host 192.168.10.239 ran 1560 1591 //разрешаем доступ к серверу для тонгких клиентов 1С

permit TCP 10.10.1.0 0.0.0.255 host 192.168.10.250 eq 139 //разрешаем доступ к серверу по SMB

permit TCP 10.10.1.0 0.0.0.255 host 192.168.10.250 eq 445 //разрешаем доступ к серверу по SMB

permit UDP 10.10.1.0 0.0.0.255 host 192.168.10.250 ran 137 138 //разрешаем доступ к серверу по SMB

permit TCP 10.10.1.0 0.0.0.255 host 192.168.10.250 ran 21 22 //разрешаем доступ к серверу по SMB

permit ICMP 10.10.1.0 0.0.0.255 192.168.10.0 0.0.0.255 //разрешаем ICMP трафик в сеть серверов

deny ip 10.10.1.0 0.0.0.255 192.168.10.0 0.0.0.255 //запрещаем весь остальной трафик в сеть серверов

deny TCP 10.10.1.0 0.0.0.255 11.11.11.0 0.0.0.255 eq 22 //запрещаем SSH к маршрутизаторам

permit ip 10.10.1.0 0.0.0.255 11.11.11.0 0.0.0.255 //разрешаем доступ остального трафика к роутерам

permit icmp any any //разрешаем весь оставшийся ICMP трафик. Например для PING в интернет.

permit TCP 10.10.1.0 0.0.0.255 any eq 80 //разрешаем HTTP трафик

permit TCP 10.10.1.0 0.0.0.255 any eq 8080 //разрешаем HTTPS трафик

permit TCP 10.10.1.0 0.0.0.255 any eq 443 //разрешаем HTTPS трафик

permit tcp 10.10.1.0 0.0.0.255 any eq pop3 //разрешаем трафик почтовых приложений

permit tcp 10.10.1.0 0.0.0.255 any eq smtp //разрешаем трафик почтовых приложений

9999 deny ip any any //визуализация дефолтного правила для отображения счётчиков

**Список доступа для сети принтеров.**

ip acc ex PRINTERS

permit udp any any eq bootps //разрешаем DHCP

permit icmp 10.10.4.0  0.0.0.255 10.10.9.0 0.0.0.255 ECHO  //разрешаем ICMP ECHO в сеть admin

9999 deny ip any any //визуализация дефолтного правила для отображения счётчиков

**Список доступа для IP телефонии**

ip acc ex TELEPHONY 

permit udp any any eq bootps //разрешаем DHCP

permit icmp 10.10.4.0  0.0.0.255 10.10.9.0 0.0.0.255 ECHO

deny ip 10.10.7.0 0.0.0.255 10.10.2.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.7.0 0.0.0.255 10.10.3.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.7.0 0.0.0.255 10.10.5.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.7.0 0.0.0.255 10.10.6.0 0.0.0.255 //запрещаем доступ в соседнюю сеть 

deny ip 10.10.7.0 0.0.0.255 10.10.1.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.7.0 0.0.0.255 10.10.8.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.7.0 0.0.0.255 10.10.9.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.7.0 0.0.0.255 10.10.10.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.7.0 0.0.0.255 192.168.10.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

9999 deny ip any any //визуализация дефолтного правила для отображения счётчиков

**Список доступа для сети управления коммутаторами**

ip acc ex MGMNT

permit icmp 10.10.4.0  0.0.0.255 10.10.9.0 0.0.0.255 ECHO //разрешаем доступ ECHO в есть ADMIN

9999 deny ip any any //визуализация дефолтного правила для отображения счётчиков

**Список доступа для сети администраторов**

ip acc ex ADMIN 

permit IP any any //Администраторы имеют доступ везде 

9999 deny ip any any //визуализация дефолтного правила для отображения счётчиков

**Список доступа для серверов**

ip acc ex SERVERS

permit udp any any eq bootps //разрешаем DHCP

permit udp 192.168.10.0 0.0.0.255 any eq domain //Разрешаем DNS

permit TCP 192.168.10.0 0.0.0.255 any eq domain //Разрешаем DNS

permit TCP 192.168.10.0 0.0.0.255 10.10.4.0 0.0.0.255 //Разрешаем доступ к принтерам

deny ip 192.168.10.0 0.0.0.255 10.10.2.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 192.168.10.0 0.0.0.255 10.10.3.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 192.168.10.0 0.0.0.255 10.10.5.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

permit ip 192.168.10.0 0.0.0.255 10.10.6.0 0.0.0.255 //Разрешаем доступ в DMZ

deny ip 192.168.10.0 0.0.0.255 10.10.7.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 192.168.10.0 0.0.0.255 10.10.8.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 192.168.10.0 0.0.0.255 10.10.9.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 192.168.10.0 0.0.0.255 10.10.10.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny TCP 192.168.10.0 0.0.0.255 11.11.11.0 0.0.0.255 eq 22 //запрещаем доступ к маршрутизаторам по SSH

permit ip 192.168.10.0 0.0.0.255 11.11.11.0 0.0.0.255 //Разрешаем доступ в сеть маршрутизации

permit icmp any any //Разрешаем остальной ICMP трафик

permit TCP 192.168.10.0 0.0.0.255 any eq 80 //Разрешаем доступ по HTTP

permit TCP 192.168.10.0 0.0.0.255 any eq 8080 //Разрешаем доступ по HTTPS

permit TCP 192.168.10.0 0.0.0.255 any eq 443 //Разрешаем доступ по HTTPS

permit tcp 192.168.10.0 0.0.0.255 any eq pop3 //Разрешаем доступ почтовых приложений

permit tcp 192.168.10.0 0.0.0.255 any eq smtp //Разрешаем доступ почтовых приложений

9999 deny ip any any //визуализация дефолтного правила для отображения счётчиков

**Список доступа для сети DMZ**

ip acc ex DMZ

permit udp any any eq bootps //Разрешаем DHCP

permit udp 10.10.6.0 0.0.0.255 host 192.168.10.253 eq domain //Разрешаем DNS

permit TCP 10.10.6.0 0.0.0.255 host 192.168.10.253 eq domain //Разрешаем DNS

deny ip 10.10.6.0 0.0.0.255 10.10.2.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.6.0 0.0.0.255 10.10.3.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.6.0 0.0.0.255 10.10.5.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.6.0 0.0.0.255 10.10.1.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.6.0 0.0.0.255 10.10.7.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.6.0 0.0.0.255 10.10.8.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.6.0 0.0.0.255 10.10.9.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.6.0 0.0.0.255 10.10.10.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny ip 10.10.6.0 0.0.0.255 192.168.10.0 0.0.0.255 //запрещаем доступ в соседнюю сеть

deny TCP 10.10.6.0 0.0.0.255 11.11.11.0 0.0.0.255 eq 22  //запрещаем доступ маршрутизаторам по SSH

permit ip 10.10.6.0 0.0.0.255 11.11.11.0 0.0.0.255 //разрешаем доступ к сети маршрутизаторов

permit icmp any any  //Разрешаем остальной ICMP трафик

permit TCP 10.10.6.0 0.0.0.255 any eq 80 //Разрешаем доступ по HTTP

permit TCP 10.10.6.0 0.0.0.255 any eq 8080 //Разрешаем доступ по HTTPS

permit TCP 10.10.6.0 0.0.0.255 any eq 443 //Разрешаем доступ по HTTPS

permit tcp 10.10.6.0 0.0.0.255 any eq pop3 //Разрешаем доступ почтовых приложений

permit tcp 10.10.6.0 0.0.0.255 any eq smtp //Разрешаем доступ почтовых приложений
...
Отказоустойчивость реализована на уровне распределения за счёт агрегации каналов и STP, что создает защиту от физического повреждения, но не обеспечивает работоспособность всей сети в случае выхода из строя одного из коммутаторов распределения.

Требование по отказоустойчивости доступа в интернет организовано на основе предоставления услуг нескольких провайдеров. Ниже приведен пример, на котором смоделировано аварийное отключение главного провайдера.

![image](https://github.com/DowningSun/OTUS/assets/156109695/5ca25c08-d7cc-4dba-a99d-ee01ad496357)



**Планы на будущее**

В будущем планируется создание сетей WI-FI в отделах, которые будут иметь несколько SSID^ соответствующий названию отдела, для подключения беспроводных рабочих мест, с доступом в соответствующий VLAN, и Гостевой сети, с доступом в VLAN DMZ.

Также будет обсуждаться возможность дублирования критической инфраструктуры, как то: дублирование Коммутатора уровня ядра и дублирование коммутаторов уровня распределения. Однако эта часть упирается в финансирование проекта.

Также планируется установка соединения с головным офисом посредствой создания IPSec VPN. 

Эту часть работы я приберегу для проф. курса.


На этом презентация окончина. Спасибо за внимание.
